...
def get_user_status(vk_id = 223134565):
    try:
        status = session.method('status.get', {'vk_id': vk_id})
        return status
...
def get_user_avatar(vk_id=223134565):
    avatar = session.method('photos.get', {'vk_id': vk_id, 'album_id': 'profile'})
    return avatar
...
def get_user_photos_saved(vk_id = 223134565):
    try:
        photos = session.method('photos.get', {'vk_id': vk_id, 'album_id': 'saved', 'count': 10, 'rev': 1})
